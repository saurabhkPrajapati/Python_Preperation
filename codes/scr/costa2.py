...
class Costa2Spider(scrapy.Spider):
    name = 'costa2'
    allowed_domains = []
    ...
    def extract_data(self, response):
        ...
        print(out_dict)
        self.toCSV.append(out_dict)




    def close(self, reason):
        print("clossing commands")
        random_keys_hed = []
        for dict_one in self.toCSV:
            for key, val in dict_one.items():
                random_keys_hed.append(key)
        keys = sorted(set(random_keys_hed))
        #sorting by key
        # keys_sorted = dict(sorted(keys.items(), key=lambda x: x[0]))
        with open(f'CA-Contra-Costa-{self.startdate.replace("/", "")}.csv', 'w', newline='')  as output_file:
